...
def main():
    ...
    print("\n=== SVM Train+Test Summary ===")
    print(f"Dataset: {ds}")
    print(f"[SVM] macro-F1: {macro_f1:.4f}\n")
    ...
    joblib.dump(
        {"model": svm, "features": X_train.columns.tolist(), "label_encoder": le},
        models_dir / "svm.pkl"
    )
    print(f"[ok] saved → {(models_dir / 'svm.pkl').resolve()}")
